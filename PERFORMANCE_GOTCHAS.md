...
API only by default; do not fall back to the GUI-bundled Tailscale CLI when the
local service is unhealthy. Where a CLI fallback remains supported, give it a
hard deadline and terminate its process group so wrapper scripts cannot leave
descendants behind. Make native startup, discovery, and pairing invokes async
and move blocking status work away from the UI thread. Treat failure as degraded
optional functionality: continue locally and render a persistent, actionable
“Tailscale needs attention” message. On macOS, automatic CLI work is disabled by
default across both the app and persistent background services; the explicit
