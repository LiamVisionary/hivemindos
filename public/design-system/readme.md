...
- **Bee role portraits (PNG).** Painted armored-bee avatars distinguish agent roles: **Queen** (crowned, the orchestrator) plus workers — Coder, Research, Planner, QA, Ops, Security, Writer, General. Copied into `assets/bees/`. Reserve the detailed art for Queen and identity/detail surfaces; dense operational maps use flat Lucide role glyphs so the portraits stay distinctive.
