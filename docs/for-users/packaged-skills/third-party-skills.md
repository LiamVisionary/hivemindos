...
| `brand/hivemindos/hivemindos-brand-visuals` | HivemindOS-authored optional identity skill for logos, sigils, emblems, and brand imagery. It grounds each direction in a concrete brand job and one visual thesis, treats hexagons as optional rather than a default, separates mark quality from mockup polish, and keeps raster generations in the concept-exploration lane. |
...
| `gtm/hivemindos/instagram-growth-system` | HivemindOS-authored optional GTM skill for evidence-led Instagram growth and monetization: account baselines, audience research, positioning and pillars, idea and sales-content banks, Reel hooks, content optimization, format-aware repurposing, and a measurable 60-day solo-creator operating system. |
...
| `media/hivemindos/ai-ugc-production-pipeline` | HivemindOS-authored optional performance-creative and media-production skill for public-ad evidence coding, honest pain/reframe positioning, five-beat scripts, storyboard-first generation, verified MiniMax H3/Higgsfield/Seedance routing, controlled variants, and performance-regeneration rules. |
| `media/hivemindos/minimax-h3-video-prompting` | HivemindOS-authored MiniMax H3 T2VA/I2VA/FL2VA/L2VA/Ref2VA prompt compiler with exact schemas, boundary-frame and full-reference semantics, dialogue/audio timing, license/territory and private-upload gates, and artifact QA. It paraphrases and links to MiniMaxAI's upstream guides rather than redistributing them. |
| `media/hivemindos/video-generator-prompting` | HivemindOS-authored exact-runtime router for generative-video prompts. It resolves provider, model, endpoint, mode, duration, anchors, and audio capabilities before loading the reviewed MiniMax H3, LTX 2.3, Seedance, Higgsfield, or other runtime guide, and records the selected guide in the generation receipt. |
...
| `media/hivemindos/content-rewards-viral-app-campaign` | HivemindOS-authored optional growth skill for performance-paid faceless AI UGC creator campaigns: app-readiness gates, viral format banks, creator scoring and course briefs, verified-view reward controls, full-funnel diagnostics, bounded cohort/format-decay loops, and a rights-cleared organic-to-paid handoff. |
...
| `productivity/madslorentzen/ai-job-search` | Security-audited optional MIT skill for a private local-first job search from profile onboarding and bounded discovery through ranking, factual CV and cover-letter drafting, PDF/ATS verification, tracking, interview preparation, outcomes, read-only Gmail review, and approval-gated Notion reporting. It includes six upstream portal clients, keeps personal state in a user-chosen private workspace, and never submits applications or sends messages automatically. |
...
| `writing/petergyang/no-ai-slop` | Security-audited optional MIT writing skill for preserving a writer's voice while removing named AI-slop patterns, or detecting those patterns without rewriting or guessing whether AI wrote the draft. It ships the upstream evaluation rubric and contains no executable code or runtime dependencies. |
