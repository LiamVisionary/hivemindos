...
Optional catalog skills are not auto-installed. Optional crypto helpers such as `packaged-skills/optional/crypto/hivemindos/b20-issuer-proof/` and the UI Skills design catalog under `packaged-skills/optional/design/<source>/<skill>/` are copied into the shared brain only when the user chooses a skill from the catalog.
