...
function Write-HivemindManagedBlock {
  ...
  $lines.Add("")
  $lines.Add("Memory writes live under ``Memory/Distillations/Agent Memory/``; the private typed-memory search index lives at ``Operations/Brain Services/Agent Memory Index.jsonl``; the generated full-vault lexical index lives at ``Operations/Brain Services/Full Vault Search Index.jsonl``; optional GitLawb receipts live at ``Operations/Brain Services/Agent Memory Proofs.jsonl`` and store hashes/provenance instead of memory bodies. Include available ``agentName``, ``agentId``, ``runtime``, ``machineName``, ``machineId``, ``tailnetId``, ``tailnetName``, ``tailnetDnsName``, ``collectorUrl``, ``sessionId``, and ``project`` fields when writing. Use ``proof: `"auto`"`` unless explicit proof is requested. Do not store raw Tailnet IPs or secrets in shared memory. ``Operations/Secure/`` reference/status notes are searchable during full-vault recall so agents can know which credential names exist or are set, but plaintext secret values must stay out of notes and responses.")
  $lines.Add("")
