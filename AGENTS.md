...
- Shared Brain Memory writes durable typed memories to `Memory/Distillations/Agent Memory/` with its private typed-memory index at `Operations/Brain Services/Agent Memory Index.jsonl`, its generated full-vault lexical index at `Operations/Brain Services/Full Vault Search Index.jsonl`, and optional hash-only GitLawb receipts at `Operations/Brain Services/Agent Memory Proofs.jsonl`.
