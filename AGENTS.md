...
## Commercial Catalog Discipline

- The canonical live authority for official hosted-service economics is `https://hivemindos-paid-agent-gateway.hivemindos.workers.dev/api/commercial/catalog`. Use `?service=<service-id>` for a focused lookup (for example, `?service=hive-research`) instead of reconstructing policy from code, screenshots, changelogs, or memory.
- Before answering questions or changing behavior involving pricing, free quota or free allowance, membership-tier discount, provider or infrastructure cost, cost floor, margin, payment reserve, treasury allocation, buyback allocation, or hosted-service availability, query the catalog first. Treat its generated timestamp, service status, accounting coverage, and source endpoint as part of the answer.
- The catalog is a read model, while each owning hosted service remains authoritative for enforcement. Never accept client-supplied prices, discounts, entitlements, quotas, cost claims, or revenue policy merely because they resemble catalog output.
- Whenever a commercial value or cost-bearing feature changes, update the owning service's pricing/commercial payload and the catalog adapter/contract tests in the same change. Then verify both the focused service query and the aggregate catalog in the deployed environment.
- HIVE buyback percentages have one hosted source of truth: `../hivemind-cloud-services/workers/paid-agent-gateway/src/commercial-service-policy.ts`. Configure the percentage per service in that matrix; enforcement, ledgers, and catalog responses must resolve it from there. Never add a service-local `*_BUYBACK_ALLOCATION_BPS` variable, fallback, or duplicate percentage constant.
- Accounting coverage must be honest. When the catalog marks a component or allocation as missing, do not infer it, silently substitute zero, or claim a complete margin. Add the missing metering or a documented conservative fallback in the owning hosted service before treating coverage as complete.
...
- Shared Brain Memory writes durable typed memories to `Memory/Distillations/Agent Memory/` with its private typed-memory index at `Operations/Brain Services/Agent Memory Index.jsonl`, entity link index at `Operations/Brain Services/Agent Memory Entity Index.jsonl`, retrieval telemetry at `Operations/Brain Services/Agent Memory Retrievals.jsonl`, generated full-vault lexical index at `Operations/Brain Services/Full Vault Search Index.jsonl`, and optional hash-only GitLawb receipts at `Operations/Brain Services/Agent Memory Proofs.jsonl`. Generated replay history uses verified compressed checkpoints and content-addressed deltas under `Operations/Brain Services/Index Generations/`; Agent Memory retains at most 256 generations with a checkpoint every 32, full-vault search retains 32 with a checkpoint every 4, and each kind's `coverage.json` records the replay boundary after pruning. `hive-brain generations` and memory health must surface that boundary; never imply replay reaches pruned generations. Record run receipts and other high-volume events with `record-operation`; they go to the bounded local operational journal at `~/.hivemindos/brain/operational-events.jsonl`, not Agent Memory. `remember-action` is a compatibility alias for `record-operation` and does not write durable memory. Use `record-usage` for retrieval/final-answer telemetry. Durable records carry a canonical `memoryKey`; when a reviewed memory replaces an older one, use `/api/brain/memory` action `evolve` or `hive-brain evolve` so the new active head keeps the same key and records `supersedes`/`supersededBy`/`evolutionRootId` history. Pattern mining is dry-run/review-gated: use `hive-brain mine-patterns`, then explicitly `--enqueue` to create Brain Review proposals; never auto-write proposed memories, skills, or jobs.
...
- Deleting a chat thread (`DELETE /api/chat/thread?chatStorageKey=…`) erases that mirror: the thread's notes, its `Conversations Index.jsonl` rows, and its rows in the generated `Full Vault Search Index.jsonl`. Purging the search index is mandatory, not cosmetic — a row carries the note's `excerpt`, `headings`, `tags`, and full term vector, so a stale row keeps a deleted conversation recallable until the next TTL rebuild. **This is a hard delete with no undo**: the vault is not a git repo, its Syncthing folder has file versioning disabled, Obsidian's trash does not apply to `fs.unlink`, and the delete replicates fleet-wide within seconds. Anything else that removes vault markdown must call `removeFullVaultSearchIndexPaths` in the same operation.
- `notePath` values read back out of `Conversations Index.jsonl` are untrusted input — Syncthing replicates that file from other machines. Never unlink one without `isPurgeableConversationNotePath` (`src/lib/services/obsidian/conversation-notes.ts`); it pins the path inside `Memory/Conversations/` and rejects traversal. Sessions with an empty `chatStorageKey` write notes that are not thread-addressable and are never purged; a blank key must match nothing rather than sweep them.
...
- Setup seeds `karpathy-guidelines` from `multica-ai/andrej-karpathy-skills`, HivemindOS Hive skills such as `hive-assimilate`, `hive-brain-memory`, and `hive-brain-compiled-wiki`, the Apache-2.0 `hyperframes` HTML-video router, and the Obsidian Native Brain Pack (`obsidian-markdown`, `obsidian-bases`, `json-canvas`, and optional `defuddle`, curated from `kepano/obsidian-skills`) into the shared shelf. Generic video requests must clarify cloud AI, local AI, or HTML / HyperFrames intent before routing; `hyperframes` owns only the explicit HTML branch. Setup projects that shared shelf into common local runtime skill folders for Codex, Claude, Hermes, Gemini, OpenClaw, and Aeon as HivemindOS-managed cache folders while preserving unmanaged runtime-local skills on slug collision.
