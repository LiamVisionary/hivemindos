...
import { recordAgentCaption, recordCallConnected, recordRuntimeFailure, recordToolCompleted, recordToolStarted, recordUserTranscript } from "./agent-call-run-events";

/**
 * Dashboard call transports: the browser-side connection hooks for every voice
 * runtime an agent call can use. Extracted from agent-call-modal.tsx so the
 * modal stays a view and each transport is a self-contained hook. This module
 * is a leaf (it imports nothing from the modal) to keep the graph acyclic.
 */
...
export const ECHO_CANCELLED_AUDIO: AudioCaptureOptions & MediaTrackConstraints = {
...
export function idleBands(phase: AgentCallPhase) {
...
export function useDashboardLiveKit(connection: AgentCallLiveKit | undefined, active: boolean, muted: boolean, voiceRunId?: string) {
...
export function useDashboardRealtime(connection: AgentCallRealtime | undefined, runtimeAgent: AgentCallRuntimeAgent | undefined, active: boolean, muted: boolean, voiceRunId?: string) {
  const [status, setStatus] = React.useState<DashboardVoiceStatus>("idle");
  const [error, setError] = React.useState("");
  const [agentCaption, setAgentCaption] = React.useState("");
  const [agentSpeaking, setAgentSpeaking] = React.useState(false);
...
export function useDashboardLocalTts(connection: AgentCallLocalTts | undefined, runtimeAgent: AgentCallRuntimeAgent | undefined, active: boolean, muted: boolean, voiceRunId?: string) {
  const [status, setStatus] = React.useState<DashboardVoiceStatus>("idle");
  const [error, setError] = React.useState("");
  const [agentCaption, setAgentCaption] = React.useState("");
  const [agentSpeaking, setAgentSpeaking] = React.useState(false);
...
export type AgentCallGeminiLive = {
  provider: "gemini-live";
  model?: string;
  voice?: string;
  instructions?: string;
  /** Which hive-env key the user picked for Gemini (server resolves the value). */
  keyEnv?: string;
  tools?: AgentCallRealtimeTool[];
};
...
function pcm16ToBase64Gemini(pcm: Int16Array): string {
  let binary = "";
  const bytes = new Uint8Array(pcm.buffer, pcm.byteOffset, pcm.byteLength);
  const chunkSize = 0x8000;
  for (let offset = 0; offset < bytes.length; offset += chunkSize) {
...
function resamplePcm16Gemini(input: Float32Array, inputRate: number, outputRate: number): Int16Array {
  const ratio = inputRate / outputRate;
  const length = Math.max(0, Math.floor(input.length / ratio));
  const pcm = new Int16Array(length);
  for (let index = 0; index < length; index += 1) {
...
/**
 * Gemini Live realtime hybrid: mints a short-lived token server-side, opens the
 * bidi WebSocket, streams 16 kHz mic PCM up and plays the 24 kHz PCM the model
 * returns. Gemini runs its own server-side VAD/turn-taking, so the client just
 * streams the mic continuously and flushes playback on `interrupted` (barge-in).
 *
 * The wire protocol here (token body, WS endpoint, setup/realtimeInput/
 * serverContent shapes) was verified live against a real key on 2026-07-06.
 */
export function useDashboardGeminiLive(
  connection: AgentCallGeminiLive | undefined,
  runtimeAgent: AgentCallRuntimeAgent | undefined,
  active: boolean,
  muted: boolean,
  voiceRunId?: string,
) {
  ...
  React.useEffect(() => {
    ...
    let connectTimeout = 0;
    let statusNow: DashboardVoiceStatus = "connecting";
    ...
    const setStatusTracked = (next: DashboardVoiceStatus) => {
      statusNow = next;
      setStatus(next);
    };
    ...
    const fail = (message: string) => {
      if (cancelled) return;
      setStatusTracked("failed");
      setError(message);
      void recordRuntimeFailure(voiceRunId, "Gemini Live session error.", message);
    };
    const send = (payload: unknown) => {
      if (!cancelled && ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(payload));
    };
    ...
    const enqueuePcm = (pcm: Int16Array) => {
      ...
      for (let index = 0; index < pcm.length; index += 1) float[index] = (pcm[index] ?? 0) / 0x8000;
      ...
      node.onended = () => {
        playing.delete(node);
        if (!cancelled && playing.size === 0) setAgentSpeaking(false);
      };
      setRemoteAudioActive(true);
      setAgentSpeaking(true);
      setAgentWorkStage("idle");
    ...
    const startMic = () => {
      ...
      micProcessor.onaudioprocess = (event) => {
        if (cancelled || mutedRef.current || !audioContext || !ws || ws.readyState !== WebSocket.OPEN) return;
        const pcm = resamplePcm16Gemini(event.inputBuffer.getChannelData(0), audioContext.sampleRate, inputRate);
        if (pcm.byteLength) {
          send({ realtimeInput: { audio: { data: pcm16ToBase64Gemini(pcm), mimeType: `audio/pcm;rate=${inputRate}` } } });
        }
      };
    };

    const geminiTools = config.tools?.length
      ? [{ functionDeclarations: config.tools.map((tool) => ({ name: tool.name, description: tool.description, parameters: tool.parameters })) }]
      : undefined;
    ...
    async function connect() {
      try {
        ...
        setError("");
        const tokenResponse = await fetch("/api/phone/cloud-voice", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            action: "gemini-live-token",
            model: config.model,
            voice: config.voice,
        ...
        const token = (await tokenResponse.json().catch(() => null)) as
          | { ok?: boolean; token?: string; wsUrl?: string; model?: string; inputSampleRate?: number; outputSampleRate?: number; error?: string }
          | null;
        if (!tokenResponse.ok || !token?.ok || !token.token || !token.wsUrl) {
          throw new Error(token?.error || "Could not mint a Gemini Live token.");
        }
        if (cancelled) return;
        outputRate = token.outputSampleRate || 24_000;
        inputRate = token.inputSampleRate || 16_000;
        audioContext = new AudioContextClass();
        await audioContext.resume().catch(() => undefined);
        if (navigator.mediaDevices?.getUserMedia) {
          localStream = await navigator.mediaDevices.getUserMedia({ audio: ECHO_CANCELLED_AUDIO }).catch(() => null);
          const track = localStream?.getAudioTracks()[0] ?? null;
          if (!cancelled) setLocalMicrophoneTrack(track);
        }
        if (cancelled) return;

        const socketModel = token.model?.startsWith("models/") ? token.model : `models/${token.model || config.model || ""}`;
        ws = new WebSocket(`${token.wsUrl}?access_token=${encodeURIComponent(token.token)}`);
        connectTimeout = window.setTimeout(() => {
          if (!cancelled && statusNow !== "connected") fail("Gemini Live did not finish connecting. Check the key and network, then retry.");
        }, 20_000);
          ...
          send({
            setup: {
              ...
              generationConfig: {
                ...
                ...(config.voice ? { speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: config.voice } } } } : {}),
              },
        ...
        ws.addEventListener("message", async (event: MessageEvent) => {
          ...
          if (message.setupComplete) {
            ...
            startMic();
            void recordCallConnected(voiceRunId, "Dashboard Gemini Live voice connected.", { model: socketModel, voice: config.voice });
            return;
          }
          ...
          if (serverContent) {
            ...
            for (const part of modelTurn?.parts ?? []) {
              if (part.inlineData?.data) enqueuePcm(base64ToPcmBytes(part.inlineData.data));
            }
            const outText = (serverContent.outputTranscription as { text?: string } | undefined)?.text;
            if (outText) {
              responseCaption = `${responseCaption}${outText}`.slice(-500);
              setAgentCaption(responseCaption);
            }
            const inText = (serverContent.inputTranscription as { text?: string } | undefined)?.text;
            if (inText) setUserTranscript(inText);
            if (serverContent.interrupted) flushPlayback();
            if (serverContent.turnComplete) {
              if (responseCaption.trim()) void recordAgentCaption(voiceRunId, responseCaption.trim());
              responseCaption = "";
              if (playing.size === 0) setAgentSpeaking(false);
            }
            return;
          }
          const toolCall = message.toolCall as { functionCalls?: Array<{ id?: string; name?: string; args?: Record<string, unknown> }> } | undefined;
          if (toolCall?.functionCalls?.length) {
            for (const call of toolCall.functionCalls) {
              const callId = call.id || call.name || "";
              if (!callId || handledCalls.has(callId)) continue;
              handledCalls.add(callId);
              const messageText = typeof call.args?.message === "string" ? call.args.message.trim() : "";
              setAgentSpeaking(false);
              setAgentWorkStage("tool");
              playCallToolChime();
              void recordToolStarted(voiceRunId, { callId, name: call.name ?? "", args: call.args ?? {} });
              const output = call.name === "ask_computer_agent" && messageText
                ? await askComputerAgent(runtimeAgent, messageText)
                : `Unknown or incomplete tool call: ${call.name}`;
              setAgentWorkStage("answer");
              void recordToolCompleted(voiceRunId, { callId, name: call.name ?? "", output });
              send({ toolResponse: { functionResponses: [{ id: call.id, name: call.name, response: { result: output } }] } });
            }
          }
        });
        ...
        ws.addEventListener("close", (event: CloseEvent) => {
          if (cancelled) return;
          if (event.code === 1000 || event.code === 1005) setStatusTracked(statusNow === "connected" ? "idle" : statusNow);
  ...
  const connected = active && Boolean(connection);
  return {
    agentCaption: connected ? agentCaption : "",
    agentSpeaking: connected ? agentSpeaking : false,
    agentWorkStage: connected ? agentWorkStage : "idle",
...
export function voiceStatusLabel(status: DashboardVoiceStatus, hasConnection: boolean) {
