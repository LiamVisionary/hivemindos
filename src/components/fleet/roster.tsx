...
function PerfBar({
  ...
  return (
    <div
      className={styles.rosterPerfRow}
      role="meter"
    ...
    >
      ...
      <span
        className={styles.rosterPerfValue}
        style={{ color: perfTone(clamped) }}
      >
        {detail ?? `${clamped}%`}
      </span>
    </div>
...
function RosterRow({
  ...
  React.useEffect(() => {
    ...
    const timeout = window.setTimeout(
      () => setSyncFixState({ status: "idle", message: "" }),
      12000,
    );
    return () => window.clearTimeout(timeout);
  }, [syncFixState.status]);
  ...
  const syncFixTooltip = [syncFixState.message, machine.syncIssue?.detail]
    .filter(Boolean)
    .join("\n\n");
  ...
  const codeNodeTitle = [
    ...
    codeNode?.nodeUrl,
    codeNode?.repoCount !== undefined
      ? `${codeNode.repoCount} repo${codeNode.repoCount === 1 ? "" : "s"}`
      : "",
    codeNode?.peerCount !== undefined
      ? `${codeNode.peerCount} peer${codeNode.peerCount === 1 ? "" : "s"}`
      : "",
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  const workingAgents = machine.agents.filter(
  ...
  const specsLine = system
    ? [
        ...
        typeof system.diskPct === "number"
          ? `disk ${Math.round(system.diskPct)}%`
          : "",
      ]
        .filter(Boolean)
        .join(" · ")
  ...
  return (
    ...
    >
      ...
      >
        <HexTile
          className={styles.rosterHexTile}
          size={22}
          tone={selected ? "honey" : "default"}
          surface="flat"
        >
          <MachineIcon
            aria-hidden="true"
        ...
        <div className={styles.rosterMachineBody}>
          <div className={styles.rosterMachineSummary}>
            <div className={styles.rosterMachineIdentity}>
              <div
                className={`${styles.rosterMachineName} flex items-center gap-1.5`}
              >
                {editingName ? (
                  <input
                    value={nameDraft}
          ...
          <div
            className={styles.rosterSpecsLine}
            style={{
              color: selected ? "var(--hex-honey-border)" : "var(--muted)",
            }}
          >
          ...
          {system ? (
            <div className={styles.rosterPerfSection}>
              ...
              {specsLine ? (
                <div className={styles.rosterSpecsLine}>{specsLine}</div>
              ) : null}
            </div>
          ) : null}
          ...
          <div className={styles.rosterAgentsLine}>
            ...
            >
              ...
              <span>
                {machine.agents.length === 0 ? "Add first agent" : "Add agent"}
              </span>
            </button>
            {visibleApps.length > 0 ? (
              <div
                className={styles.rosterAppsRow}
                aria-label={`Connected apps on ${machine.name}`}
              >
                {visibleApps.map((app) => (
                ...
                {overflowApps > 0 ? (
                  <span
                    className={styles.rosterAppBadge}
                    title={`${overflowApps} more app${overflowApps === 1 ? "" : "s"}`}
                  >
...
export function Roster({
  ...
  const [activeIssueMachine, setActiveIssueMachine] =
    React.useState<FleetMachine | null>(null);
  ...
  const activeIssueKey =
    activeIssueMachine && activeIssue?.fixAction
      ? `${activeIssueMachine.id}:${activeIssue.fixAction}`
      : "";
  const issueFixStatus =
    issueFixState.key === activeIssueKey ? issueFixState.status : "idle";
  const issueFixMessage =
    issueFixState.key === activeIssueKey ? issueFixState.message : "";
  ...
  const connectedAppsByMachine = React.useMemo(() => {
    ...
    return new Map(
      machines.map((machine) => [
        machine.id,
        apps.length ? machineConnectedApps(machine, apps) : [],
      ]),
    );
  }, [hostedApps, machines]);
  const runIssueFix = React.useCallback(async () => {
    ...
    setIssueFixState({
      ...
      message:
        data?.message ||
        "Repair started. Give Tailscale a few seconds, then refresh Fleet.",
    });
    ...
    <div className="grid gap-1.5">
      {machines.map((m) => (
        <RosterRow
          ...
          onOpenUsePodHost={
            onOpenUsePodHost ? () => onOpenUsePodHost(m) : undefined
          }
          onOpenCodeProof={
            onOpenCodeProof ? () => onOpenCodeProof(m) : undefined
          }
          onUpdateMachine={
            onUpdateMachine ? () => onUpdateMachine(m) : undefined
          }
          onRenameMachine={
            onRenameMachine ? (name) => onRenameMachine(m.id, name) : undefined
          }
          onOpenNetworkIssue={
            m.networkIssue ? () => setActiveIssueMachine(m) : undefined
          }
          onFixSyncIssue={
            m.syncIssue && onFixSyncIssue ? () => onFixSyncIssue(m) : undefined
          }
        />
      ))}
      {activeIssue && portalTarget
        ? createPortal(
            <div
              role="presentation"
                  ...
                  <>
                    <button
                      ...
                      style={{
                        ...
                        color:
                          issueFixStatus === "done" ? "#bbf7d0" : "#fde68a",
                        fontFamily: "var(--f-mono)",
                        fontSize: 12,
                        fontWeight: 800,
                        ...
                        cursor:
                          issueFixStatus === "running" ? "wait" : "pointer",
                        opacity: issueFixStatus === "running" ? 0.72 : 1,
                      }}
                    >
                      {issueFixStatus === "running" ? (
                        <LoaderCircle
                          size={15}
                          className="animate-spin"
                          aria-hidden="true"
                      ...
                      {issueFixStatus === "running"
                        ...
                        : issueFixStatus === "done"
                          ? "Repair started"
                          : activeIssue.fixLabel || "Fix automatically"}
                    </button>
                    {issueFixMessage ? (
                      <p
                        ...
                        style={{
                          margin: "10px 0 0",
                          color:
                            issueFixStatus === "failed" ? "#fecdd3" : "#bbf7d0",
                          fontSize: 12,
                          lineHeight: 1.45,
                        }}
                      >
