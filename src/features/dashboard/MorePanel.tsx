...
export function MorePanel({
  ...
  const items = useMemo<Record<MoreItemId, MoreItem>>(() => {
    ...
    return {
      ...
      memory: { id: "memory", icon: <Activity aria-hidden="true" />, eyebrow: rss ? `${rss} MB RSS` : "Fleet resources", title: "Telemetry", body: "Inspect CPU, memory, disk, network, and process health across your machines.", keywords: "telemetry resources cpu memory ram disk network processes machines load monitor", ...(rss ? { badge: `${rss} MB`, badgeTone: "honey" as BadgeTone } : {}) },
