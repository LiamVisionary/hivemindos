...
function runtimeSessionMessages(session: unknown): ChatMessage[] {
  ...
  for (const message of rawMessages.filter((item): item is { role?: string; content?: string; createdAt?: number; index?: number; billing?: unknown } => (
      typeof item === "object"
      && item !== null
...
export default function DashboardApp({ initialChatAgentId, initialChatLeaf, initialView, initialVaultPanelMode, initialWorkHistory }: DashboardAppProps = {}) {
  ...
  const { answerKanbanNeedsHuman, saveKanbanNeedsHumanApiKey } = useKanbanNeedsHumanController({ chatSetupIssue, kanbanBoardSlug, kanbanStorageBody, refreshKanbanOnce, selectedKanbanAgent, selectedKanbanTask, setKanbanError, setKanbanNotice, steerSelectedKanbanTask });
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
