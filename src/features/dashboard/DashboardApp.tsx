...
function runtimeSessionMessages(session: unknown): ChatMessage[] {
  ...
  for (const message of rawMessages.filter((item): item is { role?: string; content?: string; createdAt?: number; index?: number; billing?: unknown; type?: string } => (
      typeof item === "object"
      && item !== null
...
export default function DashboardApp({ initialChatAgentId, initialChatLeaf, initialView, initialVaultPanelMode, initialWorkHistory }: DashboardAppProps = {}) {
  ...
  useVisibilityAwarePolling({
    // Only the Memory panel (full) and the More panel (RSS/growth summary) render
    // memoryTelemetry, so don't spawn the native `ps` walk on the ~12 other views,
    // and never while the window is hidden (hiddenIntervalMs: null pauses entirely).
    enabled: hydrated && (activeView === "memory" || activeView === "more"),
  ...
  return (
    ...
    <>
      ...
      {appCompletionNotification ? (
        <DashboardAppCompletionToast key={appCompletionNotification.id} notification={appCompletionNotification} durationMs={APP_COMPLETION_TOAST_VISIBLE_MS} onDismiss={dismissAppCompletionNotification} />
      ) : null}
