...
function dedupeChatTranscript(messages: ChatMessage[]) {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (sameVisibleChatMessage(output.at(-1), message)) {
      ...
      if ((message.billing && !previous?.billing) || message.feedback) {
        output[output.length - 1] = { ...previous, billing: message.billing ?? previous?.billing, feedback: message.feedback ?? previous?.feedback } as ChatMessage;
      }
      continue;
    }
...
export default function DashboardApp({ initialChatAgentId, initialChatLeaf, initialView, initialVaultPanelMode, initialWorkHistory }: DashboardAppProps = {}) {
  ...
  useVisibilityAwarePolling({
    // fleetHostedApps only render on the Agents/fleet view (AgentsPanel); MyAppsPanel
    // has its own foundry data source. Scope the poll there so it stops fetching
    // /api/fleet/apps every 30-45s on views that never show hosted apps.
    enabled: hydrated && !chatRequestActive && activeView === "agents",
    intervalMs: 30_000,
    hiddenIntervalMs: 5 * 60_000,
