...
function runtimeSessionMessages(session: unknown): ChatMessage[] {
  ...
  for (const message of rawMessages.filter((item): item is { role?: string; content?: string; createdAt?: number; index?: number; billing?: unknown; type?: string; applicationGeneration?: unknown } => (
      typeof item === "object"
      && item !== null
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (sameVisibleChatMessage(output.at(-1), message)) {
      ...
      if ((message.processEvents?.length ?? 0) > 0 && !(previous?.processEvents?.length ?? 0)) {
        output[output.length - 1] = { ...previous, processEvents: message.processEvents, billing: message.billing ?? previous?.billing } as ChatMessage;
      }
      if (message.billing && !previous?.billing) {
...
function mergeRuntimeSessionMessages(existing: ChatMessage[], sessionMessages: ChatMessage[]) {
  ...
  for (const sessionMessage of visibleSessionMessages) {
    if (sessionMessage.role !== "assistant" || (!sessionMessage.processEvents?.length && !sessionMessage.billing)) continue;
    ...
    if ((existingMessage.processEvents?.length ?? 0) >= (sessionMessage.processEvents?.length ?? 0) && (!sessionMessage.billing || existingMessage.billing)) continue;
    enrichedExisting = [
      ...enrichedExisting.slice(0, existingIndex),
      { ...existingMessage, processEvents: sessionMessage.processEvents ?? existingMessage.processEvents, billing: sessionMessage.billing ?? existingMessage.billing },
  ...
  return (
    ...
    <>
      ...
      {appCompletionNotification ? (
        <DashboardAppCompletionToast key={appCompletionNotification.id} notification={appCompletionNotification} durationMs={APP_COMPLETION_TOAST_VISIBLE_MS} onDismiss={dismissAppCompletionNotification} onNavigate={navigateDashboardTarget} />
      ) : null}
      ...
      {activeView === "governance" ? <GovernancePanel theme={dashboardTheme === "hive-light" ? "light" : "dark"} openSkillAttachmentBrowser={openSkillAttachmentBrowser} chooseDirectoryForMachine={chooseDirectoryForMachine} defaultDirectoryMachine={localKanbanMachineTarget} /> : null}
