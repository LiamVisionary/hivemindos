...
import brainServiceStyles from "@/features/dashboard/views/brain-services.module.css";
...
import type { DashboardGBrainStatus, DashboardSyntoStatus, DashboardTradingBrainStatus } from "@/features/dashboard/dashboard-types";
...
const brainServiceClass = createStyleClass(brainServiceStyles);
function DashboardPanelLoading({ label = "Loading dashboard panel" }: { label?: string }) {
  return (
    <section className={brainServiceClass("dashboardPanelLoading")} aria-live="polite">
      <LoaderCircle aria-hidden="true" className={vaultClass("spinIcon")} />
      <div>
        <strong>{label}</strong>
        <span>Preparing the local control surface. If this takes more than a moment, refresh the page.</span>
      </div>
    </section>
  );
}
...
const VaultPanel = dynamic(() => import("@/features/dashboard/views/VaultPanel").then((mod) => mod.VaultPanel), {
  ssr: false,
  loading: (props: any) => props?.activeView === "vault" ? <DashboardPanelLoading label="Loading Brain Services" /> : null,
});
...
export default function DashboardApp({ initialView, initialVaultPanelMode, initialWorkHistory }: DashboardAppProps = {}) {
  ...
  const [skillBrowserView, setSkillBrowserView] = useState<"catalog" | "installed" | "packs" | "audit" | "write">("catalog");
  ...
  const refreshTailscaleDevices = useCallback(async () => {
    const nativeData = await getNativeTailscaleDevices();
  ...
  const refreshHiveEnv = useCallback(async () => {
    ...
    setHiveEnvStatus("");
    try {
      const nativeData = await readNativeHiveEnv();
  ...
  const refreshFleetHostedApps = useCallback(async (signal: AbortSignal) => {
    const nativeData = await getNativeFleetAppsCache<{ apps?: FleetHostedApp[] }>({ maxAgeMs: 5 * 60 * 1000 });
    if (nativeData?.apps) {
  ...
  useEffect(() => {
    const currentBadges = new Map(activeConnectedAppBadgesForShell.map((item) => [item.key, item]));
    ...
    if (previous.initialized) {
      const completedNotifications = Array.from(previous.badges.values())
        .filter((item) => !currentBadges.has(item.key))
        .filter((item) => !currentActiveTaskKeys.has(`${item.machineId}:${item.agentId}:${item.taskKey}`))
        .map((item) => ({
  ...
  }, [activeView, hydrated, refreshGbrainStatus, refreshSyntoStatus, refreshTradingBrainStatus, vaultPanelMode]);
  ...
  const ActiveAgentsPanel = activeView === "agents" ? AgentsPanel : EmptyDashboardPanel;
  const ActiveKanbanPanel = activeView === "kanban" || activeView === "history" ? KanbanPanel : EmptyDashboardPanel;
  const ActiveSchedulerPanel = activeView === "scheduler" || (activeView === "aeon" && schedulerDraftOpen) ? SchedulerPanel : EmptyDashboardPanel;
  const ActiveSwarmPanel = activeView === "swarm" ? SwarmPanel : EmptyDashboardPanel;
  const ActiveWalletPanel = activeView === "wallet" ? WalletPanel : EmptyDashboardPanel;
  const ActiveVaultPanel = activeView === "vault" ? VaultPanel : EmptyDashboardPanel;
  const ActiveUtilityPanels = isUtilityPanelView(activeView) || (activeView === "vault" && vaultPanelMode === "env") ? UtilityPanels : EmptyDashboardPanel;
  const ActiveAeonAutopilotPanel = activeView === "aeon" ? AeonAutopilotPanel : EmptyDashboardPanel;
  const ActivePhonePanel = activeView === "phone" ? PhonePanel : EmptyDashboardPanel;
  const ActiveChatPanel = activeView === "chat" || skillBrowserOpen || roleModalAgent || agentCreateMachine || chatFolderCreatorMachine
    ? ChatPanel
    : EmptyDashboardPanel;
  return (
