...
import type { DashboardGBrainStatus, DashboardSyntoStatus, DashboardTradingBrainStatus, RuntimeSessionSearchResult } from "@/features/dashboard/dashboard-types";
...
const MorePanel = dynamic(() => import("@/features/dashboard/MorePanel").then((mod) => mod.MorePanel), { ssr: false });
const AeonAutopilotPanel = dynamic(() => import("@/components/aeon").then((mod) => mod.AeonAutopilotPanel), { ssr: false });
const PhonePanel = dynamic(() => import("@/features/dashboard/views/PhonePanel").then((mod) => mod.PhonePanel), { ssr: false });
...
function isUtilityPanelView(view: DashboardView) {
  return ["maintenance", "sessions", "tools", "memory", "files", "notifications", "env", "my-apps", "more"].includes(view);
}
...
export default function DashboardApp({ initialView, initialVaultPanelMode, initialWorkHistory }: DashboardAppProps = {}) {
  ...
  const openAgentSkillBrowser = useCallback(async () => {
    ...
    await openBrainSkillBrowser({ sharedOnly: true, includeCatalog: false });
  }, [openBrainSkillBrowser]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/runtimes/availability", { cache: "no-store" })
      ...
      .catch(() => undefined);
    const runtimeCheckTimer = window.setTimeout(() => { for (const target of integrationTargetsByRuntime.values()) { if (!runtimeModelSelectionsByRuntime[target.runtime]) void refreshRuntimeIntegrations(target); } }, agentCreateMachine ? 350 : 0);
    return () => {
      cancelled = true;
  ...
  const {
    ...
    activeHeaderTitle: activeHeader.title,
  ...
  return (
    <main className="shell commandShell">
      <DashboardHeader {...{ Image, Search, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger, activeHeader, activeView, appCompletionNotification, isWorkView, kanbanBoard, navItems, notificationClass, notificationSummary, onOpenCommandPalette: () => setCommandPaletteOpen(true), setActiveView, setKanbanLoading, viewIcon }} />
      <nav className="dashboardBreadcrumbs" aria-label="Current dashboard location">
        {navigationBreadcrumbs.map((crumb, index) => (
          <button
            key={`${crumb.label}-${index}`}
            type="button"
            aria-current={index === navigationBreadcrumbs.length - 1 ? "page" : undefined}
            onClick={() => navigateDashboardTarget(crumb.target)}
          >
