...
function Masthead({
  view, companies, loading, onRefresh,
}: { view: "portfolio" | "cockpit"; companies: Colony[]; loading: boolean; onRefresh: () => void }) {
  const s = {
      ...
      {view === "portfolio" && (
        <div style={{ marginTop: 22, display: "grid", gridTemplateColumns: "1fr auto", gap: 24, alignItems: "end" }}>
          ...
          <div style={{ display: "flex", gap: 0, paddingBottom: 6 }}>
            <Big n={s.colonies} label="colonies" />
            <Big n={s.agents} label="agents" sub="0 humans" />
            <Big n={s.shipped} label="shipped" />
            <Big n={s.avgAlign + "%"} label="aligned" />
            <Big n={s.approvals} label="to approve" tone={s.approvals ? "honey" : null} last />
          </div>
        </div>
      )}
    </header>
  ...
  return (
    <div className="zhc-root" data-theme={theme} style={{ position: "relative", minHeight: "100%", background: "var(--bg-0)", color: "var(--fg)", borderRadius: 14, overflow: "hidden", border: "1px solid var(--line)" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <Masthead view={view} companies={colonies} loading={loading} onRefresh={onRefresh} />
        {error ? (
