...
export function DeliverableCard({ item, machineName, theme = "dark", layout = "card" }: {
  item: ClassifiedDeliverable; machineName?: string; theme?: Theme; layout?: "card" | "row";
}) {
  ...
  const surface: React.CSSProperties = {
    display: "flex", alignItems: layout === "row" ? "center" : "flex-start", gap: 12,
    width: "100%", textAlign: "left", textDecoration: "none", cursor: action === "none" ? "default" : "pointer",
    borderRadius: 14, padding: layout === "row" ? "10px 12px" : "13px 14px",
    border: `1px solid ${hover ? "var(--line-2)" : "var(--line)"}`,
    background: "linear-gradient(180deg, var(--bg-2), var(--bg-1))",
    boxShadow: hover ? "inset 0 1px 0 rgba(255,255,255,0.06), 0 8px 20px rgba(0,0,0,0.28)" : "inset 0 1px 0 rgba(255,255,255,0.04)",
    transform: hover && action !== "none" ? "translateY(-1px)" : "none",
