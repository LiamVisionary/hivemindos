...
function Section({ label, children, right }: { label: string; children: React.ReactNode; right?: React.ReactNode }) {
  return (
    <section className="fr-chat-context-section" style={{ display: "grid", gap: 10 }}>
      <div className="fr-chat-context-section-head" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
...
function Meta({ k, v, tone }: { k: string; v?: string; tone?: string }) {
  ...
  return (
    <div className="fr-chat-context-meta" style={{ border: "1px solid var(--line)", borderRadius: "var(--radius-sm)", background: "var(--panel-2)", padding: "8px 10px" }}>
      <div className="fr-eyebrow">{k}</div>
      <div className="fr-chat-context-value" style={{ color, fontFamily: "var(--f-mono)", fontSize: 12, marginTop: 4 }}>{v || "—"}</div>
    </div>
  );
}
...
function TaskCard({ thread, conv }: { thread: ExchangeThread; conv: ExchangeConversation }) {
  ...
  return (
    <Section label="Current task">
      <div className="fr-chat-context-card" style={{ display: "grid", gap: 11, border: "1px solid var(--line)", borderRadius: "var(--radius)", background: "var(--panel)", padding: "14px 15px" }}>
        <div className="fr-chat-context-task-meta" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <div className="fr-chat-context-title" style={{ color: "var(--fg)", fontFamily: "var(--f-display)", fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em", lineHeight: 1.35 }}>{thread.task || `Chat with ${conv.name}`}</div>
        <div className="fr-chat-context-meta-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7, marginTop: 2 }}>
...
function ScopeCard({ thread, conv }: { thread: ExchangeThread; conv: ExchangeConversation }) {
  return (
    <Section label="Scope">
      <div className="fr-chat-context-card" style={{ display: "grid", gap: 10, border: "1px solid var(--line)", borderRadius: "var(--radius)", background: "var(--panel)", padding: "14px 15px" }}>
        <span style={{ ...pill("var(--honey)", "var(--honey-soft)", "var(--honey-line)"), justifySelf: "start" }}>general chat</span>
        <div className="fr-chat-context-title" style={{ color: "var(--fg)", fontFamily: "var(--f-display)", fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em", lineHeight: 1.3 }}>{conv.name}</div>
        <div className="fr-chat-context-value" style={{ color: "var(--fg-3)", fontFamily: "var(--f-mono)", fontSize: 11.5 }}>{thread.scope || conv.sub || "Hive"}</div>
...
function Telemetry({ rows }: { rows: Array<[string, string]> }) {
  return (
    <Section label="Run telemetry">
      <div className="fr-chat-context-meta-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7 }}>
        {rows.map(([key, value]) => (
          <div key={key} className="fr-chat-context-meta" style={{ border: "1px solid var(--line)", borderRadius: "var(--radius-sm)", background: "var(--panel-2)", padding: "9px 11px" }}>
            <div className="fr-eyebrow">{key}</div>
            <div className="fr-chat-context-value" style={{ color: key.toLowerCase() === "tokens" ? "var(--honey)" : "var(--fg-2)", fontFamily: "var(--f-mono)", fontSize: 12.5, fontWeight: 500, marginTop: 4 }}>{value || "—"}</div>
          </div>
        ))}
      </div>
    </Section>
...
function QuickActions({ actions, blocked, onAction }: { actions: string[]; blocked?: boolean; onAction: (action: string) => void }) {
  ...
  return (
    <Section label="Quick actions">
      <div className="fr-chat-context-actions" style={{ display: "grid", gap: 7 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(116px, 1fr))", gap: 7 }}>
...
export function ContextPanel({
  ...
  return (
    <div className="fr-chat-context-panel">
