...
//
// Invocation (confirmed against the engine's run_cli, src/main.c:231-278):
//   <binary> cli <toolName> '<argsJson>'
// Without --json, the engine UNWRAPS the MCP envelope and prints only the inner
// tool-payload JSON to stdout (exit 0); on an engine/tool error it prints the
// error text to STDERR and exits 1. So: parse stdout on success, treat a
// non-zero exit as the error case. We never use a shell — execFile with a fixed
// argv array, a fixed/known binary path, timeouts, and a maxBuffer cap. The
// engine's JSON is treated as untrusted external data and defensively coerced.
...
function candidateBinaryPaths(): string[] {
  ...
  const envOverride = process.env.CODEBASE_MEMORY_MCP_BIN;
  if (envOverride) candidates.push(resolve(envOverride));
...
const KNOWN_NODE_KEYS = new Set([
  "name", "qualified_name", "label", "file_path", "in_degree", "out_degree",
  "start_line", "end_line", "signature", "is_exported",
]);

function normalizeNode(raw: unknown, idx: number): CodeGraphNode {
  const record = asRecord(raw);
  const name = toStr(record.name) ?? toStr(record.qualified_name) ?? `node_${idx}`;
  const qualifiedName = toStr(record.qualified_name);
  ...
  return {
    ...
    name,
    qualifiedName,
    filePath: toStr(record.file_path),
    startLine: toNum(record.start_line),
    endLine: toNum(record.end_line),
...
async function resolveProjectViaList(binary: string, repoPath: string): Promise<string | null> {
  const result = asRecord(await runTool(binary, "list_projects", {}, STATUS_TIMEOUT_MS).catch(() => ({})));
...
export class CodebaseMemoryProvider implements CodeIntelProvider {
  ...
  async detectStatus(ref: CodeIntelRepoRef): Promise<ProviderStatusInfo> {
    ...
    try {
      const status = asRecord(await runTool(binary, "index_status", { project: ref.project }, STATUS_TIMEOUT_MS));
      ...
      return { availability: "available", version, indexed: false, detail: "indexed-state probe inconclusive" };
    }
  }
  ...
  async searchGraph(input: SearchGraphInput): Promise<SearchGraphResult> {
    return this.withProject(input, async (binary, project) => {
      const args: Record<string, unknown> = { project, limit: Math.min(input.limit ?? 50, RESULT_NODE_CAP) };
      ...
      const result = asRecord(await runTool(binary, "search_graph", args, QUERY_TIMEOUT_MS));
      const nodes = asArray(result.results).slice(0, RESULT_NODE_CAP).map(normalizeNode);
      return {
        nodes,
    ...
    return this.withProject(input, async (binary, project) => {
      ...
      const result = asRecord(
        await runTool(
          binary,
          "trace_path",
          { project, function_name: input.functionName, direction, depth: input.depth ?? 5, risk_labels: true },
          QUERY_TIMEOUT_MS,
        ),
      );
      return {
        ...
        direction,
        callers: asArray(result.callers).slice(0, RESULT_NODE_CAP).map(normalizeNode),
        callees: asArray(result.callees).slice(0, RESULT_NODE_CAP).map(normalizeNode),
        hint: toStr(result.hint),
      };
    });
  }
  ...
  async getArchitecture(input: ArchitectureInput): Promise<ArchitectureResult> {
    return this.withProject(input, async (binary, project) => {
      ...
      const result = asRecord(await runTool(binary, "get_architecture", args, QUERY_TIMEOUT_MS));
      const routes: ArchitectureRoute[] = asArray(result.routes).map((raw) => {
        const record = asRecord(raw);
        return {
      ...
      return {
        ...
        nodeLabels: asArray(result.node_labels).map((raw) => {
          const record = asRecord(raw);
          return { label: toStr(record.label) ?? "", count: toNum(record.count) ?? 0 };
        }),
        edgeTypes: asArray(result.edge_types).map((raw) => {
          const record = asRecord(raw);
          return { type: toStr(record.type) ?? "", count: toNum(record.count) ?? 0 };
        }),
        languages: asArray(result.languages).map((raw) => {
          const record = asRecord(raw);
          return { language: toStr(record.language) ?? "", fileCount: toNum(record.file_count) ?? 0 };
        }),
        routes,
        entryPoints: asArray(result.entry_points).map((raw) => {
          const record = asRecord(raw);
          return { name: toStr(record.name) ?? "", qualifiedName: toStr(record.qualified_name), file: toStr(record.file) };
        }),
        hotspots: asArray(result.hotspots).map((raw) => {
          const record = asRecord(raw);
    ...
    return this.withProject(input, async (binary, project) => {
      const result = asRecord(
        await runTool(
          binary,
          "get_code_snippet",
          { project, qualified_name: input.qualifiedName, include_neighbors: input.includeNeighbors === true },
          QUERY_TIMEOUT_MS,
        ),
      );
  ...
  async detectChanges(input: DetectChangesInput): Promise<DetectChangesResult> {
    return this.withProject(input, async (binary, project) => {
      const args: Record<string, unknown> = { project, scope: input.scope ?? "symbols", depth: input.depth ?? 2 };
      if (input.since) args.since = input.since;
      else args.base_branch = input.baseRef ?? "main";
      const result = asRecord(await runTool(binary, "detect_changes", args, QUERY_TIMEOUT_MS));
      const touchedFiles = asArray(result.changed_files).map((value) => String(value));
      const affectedSymbols: CodeGraphNode[] = asArray(result.impacted_symbols).map((raw, idx) => {
        const record = asRecord(raw);
        return {
          id: `${toStr(record.label) ?? "symbol"}:${toStr(record.name) ?? idx}`,
          kind: isNodeKind(toStr(record.label)),
          name: toStr(record.name) ?? `symbol_${idx}`,
          filePath: toStr(record.file),
        };
      });
