...
export function buildWalletToolContext(wallet?: AgentWalletConfig): string {
  ...
  const lines = [
    ...
    privateTransferAssets
      ...
      : "",
    "- Tool: x402_fetch",
    `- Dashboard endpoint: call walletTools.x402Fetch when provided, otherwise POST /api/wallet/x402 with { agentId: '${wallet.agentId}', actingAgentId: '${wallet.actingAgentId || ""}', url, method, headers, body, policy, confirmation }. Keep both wallet and acting-agent ids unchanged so the server can enforce attachment and per-agent permission.`,
    ...
    wallet.autoPayEnabled
      ? `- Stablecoin sends follow the same auto-use rule: POST /api/wallet/send with agentId '${wallet.agentId}' and actingAgentId '${wallet.actingAgentId || ""}' may send USDC on Base/Solana or USDG on Robinhood Chain without another prompt while staying under the hard per-payment cap, but always state the exact send (source wallet address, destination address, network/chain, asset, amount) in your reply.`
      : `- Stablecoin sends follow the same auto-use rule: first show a clear send preview — source wallet address, destination address, network/chain, asset, and amount — then do not call POST /api/wallet/send with agentId '${wallet.agentId}' and actingAgentId '${wallet.actingAgentId || ""}' until the user explicitly supplies SEND_USDC for that exact recipient and amount. Personal (user:) wallets always require this explicit confirmation and never auto-send.`,
