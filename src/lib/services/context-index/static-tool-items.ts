...
export function hiveComputeContextIndexItem(): ContextIndexItem {
  return {
    ...
    summary: "Confidential GPU marketplace routing for chat, image, video, audio, music, 3D, embeddings, reranking, and custom model workloads.",
    tags: ["hive compute", "gpu marketplace", "inference routing", "spare gpu", "worker module", "ollama", "lm studio", "x402", "mpp", "tee", "model provider", "chat completions", "image generation", "video generation", "audio generation", "music generation", "3d generation", "earn"],
    aliases: [
      "rent out GPU",
      "rent compute",
      "earn with spare GPU",
    ...
    retrievalText: [
      ...
      "Discover typed non-chat offerings with GET /api/hive-compute/capabilities. Create a renter-key-bound draft with POST /api/hive-compute/jobs, stream encrypted inputs through /api/hive-compute/jobs/:jobId/artifacts/:artifactId, submit ciphertext through /api/hive-compute/jobs/:jobId/submit, poll or cancel /api/hive-compute/jobs/:jobId, then download and acknowledge encrypted output artifacts. The generic workload manifest supports image, video, audio, music, speech, 3D, embeddings, reranking, and namespaced custom tasks with typed MIME, byte, billing-unit, and price limits.",
      ...
      "Official jobs require gateway-verified hardware TEE attestation, encrypted prompt delivery into a confidential sidecar, and renter-only output encryption. The host OS, operator, outer worker, gateway, and artifact store receive ciphertext rather than generated content. Plaintext compatibility is limited to an explicitly non-official self-hosted gateway with HIVE_COMPUTE_SELF_HOSTED_ALLOW_NONCONFIDENTIAL=1.",
...
export function managedCloudAgentsContextIndexItem(): ContextIndexItem {
  return {
    ...
    retrievalText: [
      ...
      "Use POST /api/managed-cloud-agents action top_up to request a server-authored Base USDC quote and fund managed credits through the selected local wallet. The route enforces wallet custody, network, per-payment cap, cumulative wallet budgets, approval thresholds, exact USDC contract, amount, recipient, quote expiry, and onchain settlement. Company freeze and budgets apply only when an active company Work Board task passes companyTaskId. The hosted account credential is then stored in an encrypted local vault.",
      ...
      "Managed agents include a persistent Hermes workspace and managed inference. Use actions connect_tailnet, pair_brain, add_mcp, list_integrations, and remove_integration for explicit hosted capability promotion. Tailnet enrollment state and the cloud Shared Brain copy persist on retained storage; hosted HTTPS MCP credentials are encrypted by the managed service. A machine-hosted MCP or local file/app capability remains available only while its source machine and Tailnet bridge are online. Do not claim that connecting the cloud agent makes an offline machine available, and do not claim Tailnet, Brain, or MCP readiness until the managed agent reports it.",
    ].join(" "),
