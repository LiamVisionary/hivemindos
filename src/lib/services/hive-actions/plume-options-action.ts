...
export const plumeOptionsAction = defineHiveAction({
  ...
  contextIndex: {
    ...
    retrievalText:
      "Use plume_options to browse or manage Plume covered calls and cash-secured puts for TSLA and AMD. Call mode prepare first so the server reads the authoritative offer, series, oracle, collateral, and confirmation token from the pinned testnet contracts. Execute only after the user reviews that result, confirms they are outside Plume's restricted jurisdictions, and supplies the exact returned CONFIRM_OPTION_* token and review fingerprint. The server resolves the local signer, enforces wallet policy plus explicit active company-task policy when companyTaskId is supplied, simulates approvals and the action, and records the mined receipt. Never substitute testnet addresses for mainnet; chain 4663 remains disabled until the canonical registry and audit are public and reviewed.",
