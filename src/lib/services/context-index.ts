...
function localCliToolItems(): ContextIndexItem[] {
  return [
    ...
    },
    {
      ...
      summary: "Buy and sell stocks through Alpaca (paper or live brokerage), on-chain xStocks via Jupiter, or Robinhood Chain stock-token swaps via 0x/USDG, governed by per-trade caps, the company kill switch, and CONFIRM_BUY/CONFIRM_SELL.",
      tags: ["stock", "stocks", "shares", "equities", "trade", "trading", "buy", "sell", "alpaca", "xstocks", "robinhood-chain", "brokerage", "jupiter", "solana", "wallet", "spend", "agent", "tool"],
      aliases: [
        "buy a stock",
        "sell a stock",
        "buy shares",
      ...
      retrievalText: [
        ...
        "HTTP surface: POST /api/trading with { action: 'quote'|'execute', side: 'buy'|'sell', agentId, ticker, notionalUsd, confirmation? }. GET /api/trading returns venue readiness (Alpaca key presence, supported xStock tickers, Robinhood Chain 0x readiness + stock-token metadata) and the trade-ready agents.",
        ...
        "Venues: 'alpaca' places a market order via the Alpaca Trading API (defaults to PAPER; live only when the wallet sets alpacaPaper:false); 'xstocks' swaps USDC<->the verified xStock SPL mint via Jupiter (a sell sizes the position from the current price then swaps mint->USDC ExactIn) and signs with the agent's local Solana wallet; 'robinhood-chain' swaps USDG<->canonical Robinhood Stock Token ERC-20 contracts on chain ID 4663 through 0x and signs with the agent's local Robinhood Chain EVM wallet. Stock Token trading may be rejected upstream for legal/eligibility restrictions; surface that reason instead of routing around it.",
