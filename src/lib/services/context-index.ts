  ...
  return [
    ...
    },
    {
      ...
      retrievalText: [
        ...
        "MiroShark buyer path: POST /api/miroshark/x402 with action 'run' to pay the HivemindOS-hosted MiroShark proxy, using exactly one of prompt, url, or article plus optional deep_research and prediction_market. Use action 'suggest' for free ideas, 'status' with runId, and 'report' with runId and format 'json' or 'md'. Native chat can also run a MiroShark x402 simulation directly from prompts like 'run a sim on this polymarket: <url>' when the agent wallet Spend gate, Allow auto-use, and cap allow the expected $1.20 payment; the chat stream emits MiroShark progress events and embeds the final card payload.",
    ...
    },
    {
      ...
      retrievalText: [
        ...
        "Before paying, present the seed, expected $1.20 USDC Base payment, wallet policy cap, and whether deep research is enabled unless wallet auto-use clearly permits this exact spend.",
      ].join(" "),
