...
export function hasPendingCapabilityApproval(messages: readonly { capabilityApproval?: { status?: CapabilityApprovalStatus } }[] = []) {
  return messages.some((message) => message.capabilityApproval?.status === "pending");
}
