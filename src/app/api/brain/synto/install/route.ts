...
import { installSynto } from "@/lib/services/brain/synto";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await installSynto(body);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : "Could not install Syntho.",
    }, { status: 500 });
  }
}
