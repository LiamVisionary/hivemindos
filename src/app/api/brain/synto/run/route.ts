...
import { runSyntoPipeline } from "@/lib/services/brain/synto";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runSyntoPipeline(body);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : "Could not run the Syntho pipeline.",
    }, { status: 500 });
  }
}
