...
const autoGeneratedPullSafePaths = new Set(["next-env.d.ts"]);
...
function restoreAutoGeneratedPullSafePaths(paths) {
  const safePaths = paths.filter((path) => autoGeneratedPullSafePaths.has(path));
  if (!safePaths.length) return;
  git(["checkout", "--", ...safePaths], { stdio: "inherit" });
  console.log(`Restored generated file${safePaths.length === 1 ? "" : "s"} before pulling: ${safePaths.join(", ")}`);
}
...
function preserveChangelogPull() {
  ...
  restoreAutoGeneratedPullSafePaths(paths);
  const remainingPaths = dirtyTrackedPaths();
  if (remainingPaths.length === 0) {
    git(["pull", "--ff-only"], { stdio: "inherit" });
    return;
  }
  if (remainingPaths.some((path) => path !== changelogPath)) {
    git(["pull", "--ff-only"], { stdio: "inherit" });
    return;
  }
