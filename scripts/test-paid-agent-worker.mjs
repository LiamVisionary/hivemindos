...
const checks = [
  ...
  ["worker supports x402 builder codes", contents.worker.includes("declareBuilderCodeExtension") && contents.worker.includes("HIVEMINDOS_PAID_AGENT_BUILDER_CODE")],
  ["worker uses Coinbase CDP facilitator helper", contents.worker.includes("createFacilitatorConfig") && contents.worker.includes("CDP_API_KEY_ID") && contents.worker.includes("CDP_API_KEY_SECRET")],
  ["worker makes testnet explicit opt-in", contents.worker.includes("HIVEMINDOS_PAID_AGENT_TESTNET_MODE") && contents.worker.includes("defaultPaidAgentNetwork")],
  ...
  ["worker exposes platform fee policy", contents.worker.includes("/api/platform-fees/config") && contents.worker.includes("publicPlatformFeePolicy")],
  ["worker supports platform fee recipients", contents.worker.includes("HIVEMINDOS_PLATFORM_FEE_RECIPIENT_EVM") && contents.worker.includes("HIVEMINDOS_PLATFORM_FEE_RECIPIENT_SOLANA")],
  ...
  ["package declares x402 deps", contents.packageJson.includes("\"@coinbase/x402\"") && contents.packageJson.includes("\"@x402/core\"") && contents.packageJson.includes("\"@x402/evm\"") && contents.packageJson.includes("\"@x402/extensions\"")],
  ["wrangler uses node compatibility", contents.wrangler.includes("\"nodejs_compat\"")],
  ["wrangler disables testnet by default", contents.wrangler.includes("\"HIVEMINDOS_PAID_AGENT_TESTNET_MODE\": \"false\"") && !contents.wrangler.includes("\"HIVEMINDOS_PAID_AGENT_NETWORK\": \"eip155:84532\"")],
  ...
  ["readme documents CDP API key secrets", contents.readme.includes("pnpm wrangler secret put CDP_API_KEY_ID") && contents.readme.includes("pnpm wrangler secret put CDP_API_KEY_SECRET")],
  ["readme names downloaded app base url", contents.readme.includes("HIVEMINDOS_OFFICIAL_PAID_AGENT_BASE_URL")],
  ["readme documents platform fee policy", contents.readme.includes("GET /api/platform-fees/config") && contents.readme.includes("HIVEMINDOS_PLATFORM_FEES_ENABLED=true")],
