...
assert.match(serviceSource, /publishBankrCopyPerformance/, "the local adapter must create a public performance capability without exposing the bearer");
assert.match(serviceSource, /revokeBankrCopyPerformance/, "the local adapter must revoke public performance capabilities");
assert.match(serviceSource, /validatedPerformanceUrl/, "the local adapter must bind returned public links to the official hosted origin and monitor");
...
assert.match(panelSource, /Publish performance/, "monitor cards must expose the verified performance publication action");
assert.match(panelSource, /Rotate link/, "monitor owners must be able to invalidate an old public link");
assert.match(panelSource, /action: "revoke-performance"/, "monitor owners must be able to revoke public performance");
assert.match(panelSource, /Wallet transfers are excluded/, "the performance UI must not mislabel raw wallet activity as copied-trade PnL");
assert.match(routeSource, /"update"/, "the authenticated local route must accept subscription updates");
assert.match(routeSource, /"publish-performance"/, "the authenticated local route must proxy performance publication through the encrypted monitor credential");
assert.match(routeSource, /"revoke-performance"/, "the authenticated local route must proxy performance revocation");
