...
const personalWalletCardSource = viewSource.slice(viewSource.indexOf("function MyWalletCard"), viewSource.indexOf("function BankrWalletCard"));
assert.doesNotMatch(personalWalletCardSource, /<span className="nm"[^>]*textOverflow/, "Personal wallet names must not be silently truncated.");
assert.match(walletStyles, /\.fw-rename \.nm\s*\{[^}]*white-space:\s*normal;[^}]*overflow-wrap:\s*anywhere;/s, "Personal wallet names must wrap when the card header is narrow.");
assert.match(personalWalletCardSource, /<div className="fw-wallet-meta">[\s\S]*?<div className="sub">\{w\.kind\} · \{w\.network\}<\/div>[\s\S]*?\{w\.primary \? <Badge tone="honey">Primary<\/Badge> : null\}[\s\S]*?<\/div>/, "Primary status belongs in the identity metadata instead of taking a separate header column.");
assert.match(walletStyles, /\.fw-wallet-meta\s*\{[^}]*flex-wrap:\s*wrap;/s, "Wallet metadata must reflow without squeezing the wallet name.");
...
assert.match(nativePersonalWalletsSource, /nativeWallets \? mergePersonalWalletList\(\[\.\.\.nativeWallets, \.\.\.httpWallets\]\) : httpWallets/);
