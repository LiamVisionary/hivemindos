...
assert.match(investor, /Ordinary Bankr swaps, cross-chain actions, token launches, prediction markets, NFTs, and automations/i);
...
assert.match(staking, /15% of each service's server-owned allocation basis/i);
...
assert.match(honey, /15% of each service's server-owned allocation basis/i);
assert.match(honey, /separate exact 15% of the same server-owned allocation basis goes to the company treasury/i);
...
assert.match(investor, /server-owned per-service allocation basis/i);
assert.match(investor, /realized margin for most managed services, recognized platform revenue for others/i);
...
assert.match(investor, /tokenomics policy assigns no fixed use to the remainder/i);
...
assert.match(treasuryPrograms, /HIVE buybacks \| 15% of the per-service allocation basis/i);
assert.match(treasuryPrograms, /Company treasury \| 15% of the per-service allocation basis/i);
assert.match(treasuryPrograms, /Outside tokenomics allocation \| Remainder of the per-service allocation basis/i);
assert.match(treasuryPrograms, /realized margin for most managed services, recognized platform revenue for others/i);
