...
- Status: Pushed
- Areas changed: Tauri app icon bundle, web/favicon icon assets, Tauri loading icon, transparent social icon asset, local app-icon layer PNGs, changelog
- Summary: Replace the free-floating HivemindOS bee icon with a locally composed macOS-style rounded-square app icon built from separate shadow, glass plate, subject, and bevel/highlight layers, then inset the final plate so it matches neighboring Dock icon sizing. Keep the prior generated plate icon as a backup, add a transparent bee-and-hives social asset, and remove the wordmark and bottom orbital swirls for better Dock readability.
- Verification: Generated the transparent bee-and-hives subject with the built-in image edit path, removed the chroma key with `remove_chroma_key.py`, composed the app icon locally from layer PNGs, inset the final plate to an 824px visible box, generated the native Tauri icon bundle with `pnpm tauri icon public/app-icon-1024.png`, copied the regenerated `.icns` into the built and installed macOS app bundles, re-registered both bundles with Launch Services, and restarted Dock so macOS reloads the icon; `node -e 'JSON.parse(require("fs").readFileSync("src-tauri/tauri.conf.json","utf8")); console.log("tauri.conf.json ok")'`; Pillow dimension/alpha checks for the generated web, loading, layer, backup, and Tauri PNG icons; `file src-tauri/icons/icon.icns src-tauri/icons/icon.ico public/favicon.ico public/icons/generated/hivemindos-bee-hives-social-transparent.png`; `git diff --check -- CHANGELOG.md public src-tauri/icons src-tauri/loading/icon-192.png src-tauri/tauri.conf.json`.
