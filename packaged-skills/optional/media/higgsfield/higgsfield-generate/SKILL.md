...
description: |
  ...
  Supports two execution surfaces: Higgsfield Cloud API
  with HIGGSFIELD_API_KEY_ID/HIGGSFIELD_API_KEY_SECRET,
  and the standard consumer Higgsfield CLI/dashboard.
...
Submit jobs to Higgsfield models through the right surface for the user's intent. Supports Higgsfield Cloud API jobs and the standard consumer `higgsfield` CLI/dashboard. Covers generic image/video/3D/audio generation, Marketing Studio (branded ads, avatars, products, hooks, settings), and, secondarily, Virality Predictor video scoring.

## Surface Selection - Cloud API vs Consumer CLI

Choose the execution surface before any auth, install, upload, or generation step.

1. If the user says "Cloud", "Higgsfield Cloud", "API", "API key", "key id", "secret", "server-side", "use my env", or "no dashboard login", use Higgsfield Cloud API.
2. If the user says "consumer", "standard", "regular Higgsfield", "dashboard", "CLI", "use my logged-in Higgsfield account", or explicitly asks for the local `higgsfield` command, use the consumer CLI/dashboard.
3. If the user has not specified and both surfaces could satisfy the request, ask one short question before proceeding: "Use Higgsfield Cloud API or consumer Higgsfield CLI?"
4. Never silently switch surfaces. In particular, do not run `higgsfield auth login` for a Cloud/API-key request, and do not fall back to procedural/local generation when a Higgsfield generation was requested.

### Cloud API execution

1. Verify credentials by name only:
   ```bash
...
Only for the consumer CLI/dashboard surface:
...
The concrete commands in this section are consumer CLI examples. For Higgsfield Cloud API, keep the same creative/model choices and use the Cloud API contract selected in the surface-selection step.
