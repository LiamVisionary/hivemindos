#!/usr/bin/env python3
"""Decide whether the browser-header retry in 09-web-research.md may run.
...
Agent runtimes can use different bot tokens, so callers may pass the active
token with --agent. A 403 has two different causes: a WAF default on a site
whose published policy allows access, or a site that has actually declined.
This tells them apart.

Rules implemented (RFC 9309), deliberately on the cautious side:
  * longest-match wins; on equal specificity Disallow wins
  * a Disallow for either "*" or any supplied agent token blocks the retry
...
Usage:  python3 scripts/robots_check.py <url> [--agent <robots-token>]
...
DEFAULT_AGENT = 'HivemindOS-Agent'
...
def gate(url, agents=()):
    agents = tuple(dict.fromkeys(a.strip() for a in agents if a and a.strip()))
    if not agents:
...
def build_parser():
    parser = argparse.ArgumentParser(
        description='Check robots.txt before a browser-header retry.'
    )
    ...
    parser.add_argument(
        ...
        action='append',
        default=[],
        help='robots.txt agent token used by the active fetcher; repeatable',
    )
    return parser
...
if __name__ == '__main__':
    ...
    rc, msg = gate(args.url, args.agent)
    print(msg)
    sys.exit(rc)
