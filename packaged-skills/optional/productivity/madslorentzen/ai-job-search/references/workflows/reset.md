...
Create a timestamped recovery directory under `<job-workspace>/.archive/job-search-reset-<timestamp>/`. Move each file or application folder listed in the approved inventory into the matching recovery subtree one by one. Preserve the source folders and `documents/README.md`. Do not use globs, recursive deletion, or unresolved paths. Report the archive location so the user can restore every item.
