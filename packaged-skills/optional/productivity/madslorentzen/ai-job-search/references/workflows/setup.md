...
>
> I'll help you build your professional profile so HivemindOS agents can evaluate job postings, tailor CVs, write cover letters, and prepare you for interviews.
>
...
Then proceed to Step 3 to populate the non-skill files (`profile/00-overview.md`, `cv/main_example.tex`, `job_scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **CV language:** "Should your CVs be written in English (the default, accepted in most markets), or in your market's language?" Record the answer as a `CV language: <language>` line in profile/00-overview.md's Identity section. Cover letters always match each posting's language automatically; this setting governs the CV only. If the user is unsure, keep English and note they can re-run `/setup --section search` to change it.
...
### 1. Update `profile/00-overview.md`
...
### 8. Generate `job_scraper/search-queries.md`
...
>
> - `profile/00-overview.md` - Your full candidate profile
...
> - `job_scraper/search-queries.md` - Job search queries for `/scrape`
>
