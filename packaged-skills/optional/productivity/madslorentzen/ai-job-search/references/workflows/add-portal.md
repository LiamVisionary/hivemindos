...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   ...
   - Optionally add WebSearch/`site:` placeholder queries for that board in `job_scraper/search-queries.md` (use the `[YOUR_JOB_BOARD]` style placeholders already there) so the fallback path still covers the board if the CLI is unavailable.
