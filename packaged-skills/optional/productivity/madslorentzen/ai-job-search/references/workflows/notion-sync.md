...
1. Discover an active Notion connector/app from the current runtime's capability catalog. Determine availability from exposed tools/catalog metadata; do not run provider-specific CLI discovery commands or guess tool names. If unavailable, say that Notion is not connected and stop this optional mode.
2. Verify the connection with one cheap read-only call such as workspace search. On an authentication error, tell the user to reconnect Notion through their runtime's normal integration settings and stop. Never retry in a loop or ask for a pasted token.
3. If the connector is present but cannot authenticate in the current context, state the reason in one line and stop. Do not initiate an interactive OAuth flow inside this workflow; authentication remains the user's separate action.
