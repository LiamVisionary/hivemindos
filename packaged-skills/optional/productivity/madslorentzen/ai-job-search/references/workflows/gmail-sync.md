...
Discover an active Gmail connector/app from the current runtime's capability catalog. It must support bounded thread search and full-message reads. If unavailable or unauthenticated, tell the user Gmail is not connected and stop this optional mode; do not fall back to shell mail clients, IMAP, credential requests, or guessed tool names.
