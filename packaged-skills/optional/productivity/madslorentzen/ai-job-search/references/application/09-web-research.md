...
The active web fetcher may identify itself with a runtime-specific bot token and should honor `robots.txt`. That is the formal opt-out a site owner can rely on, so a 403 has two very different causes and they must not be treated the same:
...
- **A site that has actually declined.** If `robots.txt` disallows the path for `*` or for the active fetcher's bot token, retrying with browser headers circumvents the site's expressed preference. **Do not retry.** Skip to escalation step 3 and find the employer's own posting instead.

Check it first. It is one cheap fetch, and the repo ships the check:

```bash
python3 '<skill-root>/scripts/robots_check.py' '<URL>' --agent '<runtime-agent-token>'
```

Exit status `0` means the retry may proceed; `1` means it must not, so go to escalation step 3. Pass the active fetcher's published robots token when known; otherwise omit `--agent` and the checker uses the conservative `HivemindOS-Agent` token. The rules are deliberately cautious: longest-match wins, a tie between `Allow` and `Disallow` goes to `Disallow`, and a disallow for either `*` or any supplied token blocks the retry. A `404` means no policy is published; any other failure to read `robots.txt` leaves permission unconfirmed and the retry does not happen.

Two details worth knowing, both covered by `tests/test_robots_check.py`:

- **The WAF may block `robots.txt` too.** The checker reads the policy with a browser header only when the declared bot request is refused, then obeys the returned policy strictly. If the policy still cannot be confirmed, the checker fails closed.
...
2. **Check `robots.txt`, then use the approved browser-header fetch** (above), then strip tags. This addresses the 403 class of failure. If `robots.txt` disallows the path for `*` or the active runtime token, **skip this step entirely** and go to step 3.
