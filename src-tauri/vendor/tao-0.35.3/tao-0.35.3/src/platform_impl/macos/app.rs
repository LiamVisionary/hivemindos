...
pub static APP_CLASS: Lazy<AppClass> = Lazy::new(|| unsafe {
  ...
  // HivemindOS patch: do NOT install the `sendEvent:` override. On macOS 26
  // it aborts startup with "panic in a function that cannot unwind". The
  // override only adds Cmd+key keyUp forwarding and device-motion events;
  // AppKit's native dispatch is sufficient for the dashboard.
