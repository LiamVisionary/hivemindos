...
- **Canonical commercial catalog:** before answering or changing anything about pricing, free quota or allowance, membership discounts, provider/infrastructure cost, margins, reserves, buybacks, treasury allocation, or hosted availability, query `https://hivemindos-paid-agent-gateway.hivemindos.workers.dev/api/commercial/catalog`; use `?service=<service-id>` for a focused lookup. Whenever a cost-bearing feature or commercial policy changes, update the owning service payload plus the catalog adapter/contracts in the same change and verify production. If accounting components are marked missing, do not infer them or treat them as zero. HIVE buyback percentages are configured per service only in `../hivemind-cloud-services/workers/paid-agent-gateway/src/commercial-service-policy.ts`; never add a service-local `*_BUYBACK_ALLOCATION_BPS` knob, fallback, or duplicate percentage constant.
- **We build both sides (founder project):** the hosted infrastructure is ours too — its source lives in the private repo checked out at `../hivemind-cloud-services` (e.g. `workers/compute-gateway/` with its own test suites). Hosted/gateway work is never "out of scope"; when a feature spans both, change both sides contract-compatibly in the same session and run each repo's own tests. The trust boundary decides where code lives, not whether to do the work. See AGENTS.md → Open Source And Commercial Boundary.
...
- **Runtime detection portability:** never hardcode a developer's absolute home path, machine-specific install folder, or a runtime's private venv/internal layout. Runtime/setup detectors should resolve from the current user's home/env/PATH, share helpers where possible, update dashboard availability + adapters + native setup/installer + MCP registrars together, and be checked with a sparse GUI-like `PATH`.
